...
```bash
docker exec threat_hunt_backend curl -s http://zap:8080/json/core/view/version?apikey=[REDACTED_HISTORICAL_ZAP_KEY] | head -20
...
2. Verify ZAP is listening: `docker exec zap_scanner wget --spider http://localhost:8080/json/core/view/version?apikey=[REDACTED_HISTORICAL_ZAP_KEY]`
...
- ZAP API key mismatch → both should be `[REDACTED_HISTORICAL_ZAP_KEY]`
