...
## Context & Warning
The legacy ZAP API key (`[REDACTED_HISTORICAL_ZAP_KEY]`) and previous administrator password (`[REDACTED_HISTORICAL_ADMIN_PASSWORD]`) were eliminated from the active tree and are completely absent from `HEAD`. However, they still exist in the diffs of past historical commits (e.g. `85bd576`, `c664df2`, `4e29764`, `4d8be8d`, `0c02c32`), visible via `git log -S`.
...
[REDACTED_HISTORICAL_ZAP_KEY]==>[REDACTED_HISTORICAL_ZAP_KEY]
[REDACTED_HISTORICAL_ADMIN_PASSWORD]==>[REDACTED_HISTORICAL_ADMIN_PASSWORD]
EOF
```
