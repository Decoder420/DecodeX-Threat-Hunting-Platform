...
from .db import (
    get_db, Alert, Event, User, FeedSource, SuppressionRule, IOC, ROLE_RANK,
...
def get_dashboard():
    ...
    return jsonify({
        ...
        "feeds": [{"name": f.name, "type": "ip" if "IP" in f.name else "domain", "enabled": "Yes" if f.enabled else "No"} for f in feeds],
        "charts": {
...
def get_alert_context(alert_id):
    ...
    return jsonify({
        ...
        "assigned": alert.assigned_to or "Unassigned",
...
def update_alert_case_route(alert_id):
    ...
    updated_alert = db.query(Alert).get(alert_id)
...
def save_rule():
    data = request.json
    path = os.path.join(RULES_DIR, data.get("file"))
